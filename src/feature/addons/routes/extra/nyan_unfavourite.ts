...
import { RouteNyanUnfavouriteOptions } from "../types";
...
class RouteNyanUnfavourite extends APIRoute {
    options: RouteNyanUnfavouriteOptions;

    constructor(options: RouteNyanUnfavouriteOptions) {
        super(options);
        ...
        feature.instance.post(this.path,
            ...
            async (req: Request, rep) => {
                ...
                if(user.favourites.includes(req.query.id)) {
                    user.favourites.splice(user.favourites.indexOf(req.query.id), 1);
...
export default RouteNyanUnfavourite;
